...
1. **可视化资源排程 (Visual Scheduling)**
   ...
   - **直观交互与悬停体验**：采用 Bootstrap 与原生 JS 实现的响应式网格布局，支持新增、修改和快捷编辑排班。优化了排班网格在悬停时的左右高亮与多日中间虚线框表现，视觉效果更加协调。
   - **轻量化修改预览**：重构了批量修改/拖拽调整排班时的“修改预览”展现形式，相比以往占用更少的垂直高度，同时呈现更加直观。
   - **项目工作范围细分 (Work Scopes)**：支持为项目创建多个工作范围（如“前端开发”、“后端开发”、“UI设计”等），排程预订可精确关联具体范围，并在日历中以 `项目 - 工作范围` 形式直观展示。
   - **预订状态区分**：支持“正式排程”与“临时/意向排程”（Tentative, 界面上以虚线边框样式呈现），方便项目前期筹备规划。
   - **请假与假期整合**：中国法定节假日（包含放假和调休）自动高亮显示，支持员工批量请假登记，防止排程冲突。
   - **一键预订法定节假日**：在新建预订时，可指定日期范围并一键将该周期内所有周一至周五的法定节假日自动录入为法定假期，免除手动逐天排程的繁琐操作。

2. **工时表与实际追踪 (Timesheets & Actuals)**
   - **工时填报**：员工可以按天、按项目以及具体工作范围记录实际消耗的工时，并支持填写备注信息。
   - **工时表级别唯一性约束**：底层支持灵活的联合唯一索引限制，可有效避免同一人、同一天在同项目/同工作范围下重复报工。
...
3. **多维度数据报表 (Reports & Analytics)**
   ...
   - **项目投入报表**：按客户和项目维度统计累计工时，支持按人员以及**工作范围 (Scope)** 两个维度进行下钻钻取（Drill-down）明细分析，方便精准度量各阶段工时实际占比。
...
5. **企业级协作与权限**
   ...
   - **企业时区管理 (Company Timezone)**：支持在企业管理中自主选择和配置公司统一定时区（如北京、东京、伦敦、纽约、洛杉矶及 UTC 等）。保存后，排定详情卡片中的创建与修改时间戳会按配置时区进行动态换算展示，而非单纯的 UTC 或本地浏览器时间，确保跨地区团队信息对齐。
   - **项目协同管理 (Co-management)**：支持为项目经理（Manager）分配多个协同管理项目。协同经理拥有该项目的排班管理、工作范围维护及报表钻取与导出等全部操作权限，适合大中型项目协同运作。
...
系统共包含 13 张核心表，全部定义在 [db/schema.js](file:///Users/kresnikwang/Work/crewboard/db/schema.js) 中：

1. **`enterprises` (企业租户表)**
   - 记录企业名称、企业代码 (code)、所有者 (owner_id)、三方 Webhook 链接和企业微信应用配置（ID、Secret、AgentID）。
2. **`users` (用户账号表)**
   - 记录用于登录的手机号、邮箱、密码 Hash（加盐 scrypt 算法）、绑定的资源 ID (`resource_id`)、系统角色（`role`：`admin`/`manager`/`basic`）及管理权限细项，并包含 `managed_project_ids` 字段（JSON 格式的项目 ID 数组）用于存储该用户协同管理的项目。
3. **`resources` (人员/资源表)**
...
5. **`project_scopes` (工作范围表)**
   - 记录各项目的细分工作内容/任务阶段（如前端开发、UI设计、测试等）。
6. **`bookings` (排班记录表)**
   - 排班的核心数据，按天记录员工绑定的项目、工作范围（`project_scope_id`，可为空）及对应的小时数（支持非 8 小时的零碎排班）。
7. **`timesheets` (工时记录表)**
   - 记录员工每日在各项目和对应工作范围上实际填报的工时。包含联合唯一索引（分为 `idx_timesheets_unique_scope` 和 `idx_timesheets_unique_null_scope`），支持按工作范围记录，防止在同一天、同一项目、同一工作范围内重复报工。
8. **`leave_entries` (休假记录表)**
   - 记录员工的具体请假类型（如年假、事假、病假等）与日期。在排班日历中表现为灰色斑马线高亮。
9. **`sessions` (会话 Token 表)** & **`invitations` (成员邀请表)** & **`join_requests` (加入企业申请表)** & **`password_reset_tokens` (重置密码凭证表)**
...
| **人员/资源管理** | 全权（增/删/改/活跃状态） | 只读（支持为项目经理用户分配协同管理项目） | 无权访问 |
| **客户与项目** | 全权创建、修改和归档 | 只能创建/修改自己负责或协同管理的项目 | 只读 |
| **可视化排班** | 任意排班添加与修改 | 只能为自己负责或协同管理的项目安排人员 | 只读（只能看自己的排班） |
| **工作范围管理 (Work Scopes)** | 全权添加/编辑/删除工作范围 | 只能对自己负责或协同管理的项目编辑工作范围 | 无权访问 |
| **工时表** | 全权查看，代他人录入工时 | 查看报表 | 只能填报及编辑自己的工时 |
| **数据报表** | 查看并导出所有项目与人员报表 | 只能查看自己负责或协同管理项目的报表 | 无权访问 |
